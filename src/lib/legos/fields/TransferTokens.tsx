...
export const TransferTokens = (props: Buildable<Field>) => {
  ...
  useEffect(() => {
    const getDAOTokens = async (_dao: DaoItem, chainId: ValidNetwork) => {
      const client = createViemClient({ chainId });
      const sharesDecimals = await client.readContract({
        abi: LOCAL_ABI.ERC20, address: _dao.sharesAddress as `0x${string}`, functionName: 'decimals',
      });
      const lootDecimals = await client.readContract({
        abi: LOCAL_ABI.ERC20, address: _dao.lootAddress as `0x${string}`, functionName: 'decimals',
      });
      setSelectOptions([
        { name: `${_dao.shareTokenName} (Voting Tokens)`, value: _dao.sharesAddress, paused: _dao.sharesPaused, decimals: Number(sharesDecimals) },
        { name: `${_dao.lootTokenName} (Non-Voting Tokens)`, value: _dao.lootAddress, paused: _dao.lootPaused, decimals: Number(lootDecimals) },
      ]);
    };
    if (!dao || !daoChain) return;
    getDAOTokens(dao, daoChain as ValidNetwork);
  }, [dao, daoChain]);
  ...
  const newRules: RegisterOptions = useMemo(() => ({
    ...
    validate: {
      ...
      maxValue: (value) =>
        Number(value) <= Number(handleBaseUnits(tokenBalance, 18)) || 'Cannot exceed current token balance',
      nonZero: (value) => Number(value) > 0 || 'Value must be greater than zero',
    },
    ...props.rules,
  ...
  return (
    <>
      <WrappedInputSelect
        {...props}
        placeholder="0"
        error={selectedToken?.paused ? { type: 'error', message: tokenErrorMsg } : undefined}
        selectId={selectedTokenId}
        selectPlaceholder="--"
        options={selectOptions || []}
        rightAddon={<Button color="secondary" size="sm" onClick={setMax}>Max: {tokenBalance}</Button>}
        rules={newRules}
      />
      <WrappedInput
        address
        ...
        rules={{
          required: true,
          validate: {
            ethAddress: (value) => ignoreEmptyVal(value, ValidateField.ethAddress),
            nonZeroAddress: (value) => value !== ZERO_ADDRESS || 'Cannot send to the Zero Address',
          },
        }}
      />
    </>
